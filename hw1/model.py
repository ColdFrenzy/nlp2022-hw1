...
class FCModel(nn.Module, Model):
    def __init__(self,
        ...
        network = OrderedDict()
        network["fc_input_layer"] = nn.Linear(self.input_shape, fc_layers[0])
        ...
        network["fc_output_layer"] = nn.Linear(fc_layers[-1], self.num_classes)
...
class BiLSTMCRFModel(nn.Module, Model):
    def __init__(self,
        ...
        self.num_classes = num_classes
        self.bilstm = nn.LSTM(input_features, hidden_size//2,bidirectional=True,batch_first=batch_first)
        
        self.hidden_size = (hidden_size//2)*2
        self.fc_layer = nn.Linear(self.hidden_size,num_classes)
    ...
    def extract_features(self, inputs):
        ...
        """
        B, M, F = inputs.shape
        pad_tensor = torch.zeros(F)
        masks = torch.full((B,M), True)
        for b in range(B):
            for m in range(M):
